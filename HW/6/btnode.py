...
class Sequence(BTNode):
	...
	def execute(self, delta = 0):
		...
		if self.getNumChildren() == 0 or self.getCurrentIndex() >= self.getNumChildren(): return True
		child = self.getChild(self.getCurrentIndex()).execute(delta)
		if child: self.setCurrentIndex(self.getCurrentIndex() + 1)
		elif child is False: return False
		return None
...
class Selector(BTNode):
	...
	def execute(self, delta = 0):
		BTNode.execute(self, delta)
		if self.getNumChildren() == 0 or self.getCurrentIndex() >= self.getNumChildren(): return False
		child = self.getChild(self.getCurrentIndex()).execute(delta)
		if child is False: self.setCurrentIndex(self.getCurrentIndex() + 1)
		elif child: return True
		return None