...
class Agent:
    ...
    # The training or testing episode will run for no more than this many time steps
    numSteps = 500
    ...
    def updateVtable(self, newState, lastState, action, reward, terminal, availableActions):
        if terminal: self.v_table[lastState][action] += self.learningRate * (reward - self.v_table[lastState][action])
        else:
            # Among all available actions, find the highest utility of performing one of them
            maxUtil = -1000000
            for ind in range(len(self.v_table[newState])):
                if ind in availableActions and self.v_table[newState][ind] > maxUtil: maxUtil = self.v_table[newState][ind]
            # Use update formula alongside maxUtil to update entry
            self.v_table[lastState][action] += self.learningRate * (reward + self.gamma * maxUtil - self.v_table[lastState][action])
        return None
    ...
    def egreedy(self, observation):
        # Return random action if random number is below epsilon, otherwise use greedy
        return random.randint(0, self.numActions - 1) if random.uniform(0, 1) < self.epsilon else self.greedy(observation)
    ...
    def greedy(self, observation):
        ...
        # Find highest Q value, then return the action that has that Q value
        bestQ = max(self.v_table[tuple(observation.worldState)])
        for ind in range(len(self.v_table[tuple(observation.worldState)])):
            if self.v_table[tuple(observation.worldState)][ind] == bestQ: return ind
