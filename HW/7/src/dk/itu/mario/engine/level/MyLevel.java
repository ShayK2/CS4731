...
public class MyLevel extends Level{
    ...
    public MyLevel(int width, int height) { super(width, height); }
    ...
    public void create(MyDNA dna, int type) {
        // flat path from beginning to end
        ...
        random = new Random();
        // At each spot along the level, create a coin, rock barrier, or random enemy (enemies 4x likely, coins 2x likely than rocks)
        for (int i = 0; i < dna.getChromosome().length(); i++) {
            char spot = dna.getChromosome().charAt(i);
            if (spot == 'c' || spot == 'd') this.setBlock(STARTOFFSET + i, DEFAULTHEIGHT - 3, COIN);
            else if (spot == 'r') for (int j = -3; j > -7; j--) this.setBlock(STARTOFFSET + i, DEFAULTHEIGHT + j, ROCK);
            else if (spot == 'e' || spot == 'f' || spot == 'g' || spot == 'h') this.setSpriteTemplate(i + STARTOFFSET, DEFAULTHEIGHT - 3, new SpriteTemplate(random.nextInt(3), false));
        }
